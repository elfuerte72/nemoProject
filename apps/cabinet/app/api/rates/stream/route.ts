    ...
    start(controller) {
      ...
      const send = (chunk: string): void => {
        ...
        try {
          ...
          /*
           * Вкладку закрыли между проверкой и записью. Обычно следом
           * придёт отмена потока или обрыв запроса, но полагаться на
           * это нельзя: не придя, они оставили бы вдохи бить в закрытый
           * поток, а подписку — держать тикер ради ушедшего зрителя до
           * самого потолка жизни соединения.
           */
          writable = false;
          close();
        }
