...
describe('куда указывает имя', () => {
  ...
  it.each([
    ...
    // Разбор безопасности 14 сентября 2026: служебные диапазоны, которых
    // в правиле не было, — стенды производительности, протокольные
    // назначения IANA, NAT64 и IPv4 внутри IPv6 без приставки ffff.
    '198.18.0.1', '198.19.255.255', '192.0.0.8', '64:ff9b::a00:1', '::127.0.0.1',
