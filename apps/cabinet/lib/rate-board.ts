...
export function flashes(
  ...
  for (const one of next) {
    ...
    if (!was || !now) continue;
    /*
     * Равенство — числом, а не записью: «83» и «83.000000000000000000»
     * это один курс, пришедший из `numeric(38, 18)` разными путями.
     * Сравнив их как строки, табло подсветило бы строку красным —
     * единственным цветом, который мерчант читает как плохую новость, —
     * там, где не изменилось ничего.
     */
    const order = Money.compare(now, was);
    if (order === 0) continue;
    marks[pairKey(one)] = order > 0 ? 'up' : 'down';
  }
  return marks;
}
 ...
 *
 * Сравнивается не только курс. Отметка времени — новость сама по себе:
 * от неё считается возраст котировки в строке. Котировка целиком —
 * потому что по ней табло считает сумму: администратор правит верхнюю
 * ступень сетки, опорный курс на минимуме направления не двигается, у
 * пары от банка и отметка суточная, — и табло, сравнивающее один курс,
 * считало бы по вчерашним ступеням до самой ночи, расходясь с тем, что
 * запишет заявка.
 *
...
export function sameRates(
  ...
  return left.every((one, index) => {
    ...
    return (
      ...
      one.minAmountUsd === other.minAmountUsd &&
      // Котировка собирается из одних и тех же полей в одном месте
      // (`toQuote`), поэтому сравнивать её записью законно, а перебирать
      // ступени руками значило бы завести второе описание их состава.
