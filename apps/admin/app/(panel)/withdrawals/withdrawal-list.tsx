...
export function WithdrawalList({
   ...
   *
   * Причина набирается только в раскрытой строке, и держит обновление
   * сама раскрытая строка — пока она в очереди. Набранные причины
   * отдельно не считаются: они остаются в памяти экрана и после отказа, и
   * после «Отмены», и до 17 сентября 2026 с первого же отказа с причиной
   * очередь переставала обновляться сама до перезагрузки.
   */
  const typing =
    (pending !== undefined && requests.some((request) => request.id === pending.id)) ||
    editing !== undefined;
