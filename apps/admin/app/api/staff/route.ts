 ...
 * сброс — и только тому администратору, который их выполнил. Дальше он
 * либо передаёт секрет сотруднику лично, либо не передаёт вовсе: тот же
 * секрет сотрудник получит на своём первом входе, пока им ни разу не
 * входили (`packages/core/src/staff.ts`).
 */
