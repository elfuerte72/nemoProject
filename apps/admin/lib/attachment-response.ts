...
function headersFor(
  ...
  return {
    ...
    /*
     * Показанное в строке — в песочнице. Картинка сама по себе инертна, а
     * PDF нет: просмотрщик Firefox — это код на странице, и подложенный
     * клиентом файл дотягивался бы до сессии менеджера и до маршрутов,
     * которые расшифровывают реквизиты, — прямо в домене панели.
     * Песочница отнимает у показанного файла происхождение; встроенный
     * просмотрщик Chrome от неё не страдает, а там, где страдает, файл
     * скачивается — и это верный размен.
     */
...
function safeName(name: string | null): string | undefined {
  ...
  const cleaned = name
    ...
    .replace(/[\u200e\u200f\u202a-\u202e\u2066-\u2069]/g, '')
    .trim();
  const short = shorten(cleaned);
  return short === '' ? undefined : short;
}

/**
 * Длинное имя — короче, но со своим хвостом.
 *
 * Режется по знакам, а не по их половинкам: обрезка посреди эмодзи
 * оставляет половину пары, а она не кодируется в заголовок — и файл, с
 * которым всё в порядке, отвечал бы отказом. Расширение при этом
 * остаётся на месте: обрезанный «договор.docx» без него не открывается
 * двойным щелчком, а дописать его обратно панели неоткуда — тип
 * незнакомый.
 */
function shorten(name: string): string {
  const letters = [...name];
  if (letters.length <= LONGEST_NAME) return name;

  const suffix = EXTENSION.exec(name)?.[0] ?? '';
  const stem = letters.slice(0, LONGEST_NAME - [...suffix].length).join('');
  return `${stem}${suffix}`;
}
