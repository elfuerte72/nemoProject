...
interface TelegramWebApp {
  ...
  /**
   * Кто открыл приложение: `ios`, `android`, `tdesktop`, `macos`,
   * `weba` и прочие имена самого Telegram. Старый клиент не кладёт его
   * вовсе.
   */
  platform?: string;
  /**
   * Развернуть на весь экран, убрав шапку Telegram. Появился в Bot API
   * 8.0; в старых клиентах его нет, и вызов молча ничего не делает —
   * это рабочий исход, а не поломка.
   *
   * Спрашивается он не у всех, а только у телефона (`wantsFullscreen`):
   * настольный Telegram просьбу понимает и разворачивает Mini App во
   * весь монитор, закрывая собой переписку, из которой клиент пришёл.
   *
...
/**
 * Телефоны, у которых экрана мало настолько, что шапка Telegram стоит
 * строки формы. Имена — как их называет сам Telegram.
 */
const HANDHELD = new Set(['ios', 'android', 'android_x']);

/**
 * Просить ли полный экран у клиента с такой платформой.
 *
 * Список из телефонов, а не из настольных клиентов: имена вторых
 * прибавляются (`tdesktop`, `macos`, три разных веба), и незнакомое имя
 * должно означать окно. Развернуть на весь монитор того, кто этого не
 * просил, дороже, чем не развернуть телефон, у которого просто
 * останется шапка.
 */
export function wantsFullscreen(platform: string | undefined): boolean {
  return HANDHELD.has((platform ?? '').trim().toLowerCase());
}
