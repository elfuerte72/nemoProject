 ...
 *
 * У наличных он такой же: цена — то единственное, за чем открывают
 * обменник, и заставлять клиента спрашивать её в чате значит терять его
 * на первом экране. Считается она по наличной сетке ставок, если
 * администратор завёл её в панели, и по наценке сервиса, если не завёл.
 * Без курса наличная заявка остаётся там же, где и всякая другая, — где
...
export function ExchangeScreen({
   ...
   */
  const payoutMethod = useMemo((): PayoutMethod | undefined => {
    // У наличной сделки способ один — из рук в руки, — и ставка у него
  ...
  useEffect(() => {
    /*
     * Курс спрашивается и у наличных. Ставка у них бывает своя — тогда
     * сервер считает по ней; не заведена — считает по наценке, как и
     * перевод.
     */
    if (!fromCode || !toCode) {
    ...
    const ask = () => {
      ...
      void get<{ quote: QuoteView | null }>(
        ...
        .catch((failure: unknown) => {
          if (cancelled) return;
          /*
           * Кроме одного случая: подпись запуска живёт сутки, а Mini App
        ...
        <>
          ...
          */}
          ...
          <p className="hint">
            ...
            */}
            {rate === undefined
              ? // Ответ ещё не пришёл. Обещать нечего: у наличных курса
                // может не оказаться вовсе, и «курс фиксируется»,
                // мелькнувшее до ответа, обещало бы то, чего нет.
                ''
              : rate === null
                ? electronic
                ...
                : // Без указания на курс: строка над подсказкой могла ещё
                  // не набраться, и «по этому курсу» показывало бы на
                  // пустое место. Обещание при этом то же самое — оно
                  // относится к курсу заявки, а не к числу на экране. У
                  // наличных обещание такое же: названный курс сервис
                  // держит, менеджер поверх не назначает.
          ...
          <p className="sheet__body">
            {sheet.exchange === 'electronic'
              ? sheet.rate
                ...
                : // Молчащий источник — рабочее состояние: заявку по нему
                  // подают, и курс называет менеджер. Про наличные здесь
                  // говорить нельзя — человек отправляет перевод.
                  'Менеджер возьмёт заявку, назовёт курс и выдаст реквизиты для оплаты.'
              : sheet.rate
                ? 'Курс зафиксирован. Менеджер возьмёт заявку и назовёт, где и когда получить наличные.'
