...
/*
 * Контур знака вынесен из разметки, потому что рисуют его двое.
 *
 * Кроме самого `TobeeMark` тот же контур берёт заставка: там знак не
 * нарисован, а набран толщей слоёв, и каждый слой вырезан по этой же
 * форме. Оставь контур внутри компонента — заставке пришлось бы завести
 * свою копию, и правок о том, как выглядит знак, стало бы две.
 */
 ...
 * нарисован.
 *
 * Знак в своей сетке стоит с полями: по ширине занимает чуть больше
 * трети, по высоте — три пятых. Заставке нужен именно габарит: слой
 * толщи растянут по своему прямоугольнику, и возьми она полную сетку,
 * знак съёжился бы вчетверо и уехал от собственного света.
 */
