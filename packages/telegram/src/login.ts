 ...
 *
 * Живёт в общем месте, потому что виджетом входят двое: менеджер в
 * панели — первым фактором, — и амбассадор в своём кабинете
 * (docs/adr/0022). Копия этой проверки в каждом приложении — это два
 * места, где её можно написать «помягче», и разойтись они успели бы
 * на первой правке.
 *
 ...
 * Токен при этом у каждого свой: панель подписывает бот входа,
 * кабинет — клиентский бот; чей виджет нажали, тот и подписывает.
 *
 * Успешная проверка означает только «этим Telegram-аккаунтом владеет
 * тот, кто нажал кнопку». Права она не даёт: у менеджера допуск
 * выдаётся по списку сотрудников и вторым фактором, у амбассадора —
 * отметкой в программе.
 */
