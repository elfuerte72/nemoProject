 ...
 *
 * Направление выбирает тот, кто округляет: у денег оно зависит от того,
 * кто по этой сделке платит, и «к ближайшему» означало бы отдавать часть
 * наценки случаю. Порог суммы на черте курса — вверх: «от 43 811 ₽»
 * называет сумму, которой уже хватает, а вниз назвал бы ту, которой ещё
 * мало. Курс до 28 августа 2026 округлялся ими же до рубля; теперь он
 * округляется до сотых (`roundRate` в `rate.ts`).
 */
 ...
 *
 * Для величин, у которых целое уже получено, но прошло через деление и
 * вернулось хвостом ограниченной точности — 81,999999999999998 вместо
 * 82. Показывать такое нельзя, а отбрасывать вниз тем более: получилось
 * бы 81. Курс, который хранится частным, снимает такой хвост сам
 * (`rate.ts`).
 */
