...
/**
 * Округление до точности валюты — то, каким сервис называет сумму
 * выдачи.
 *
 * Знак берётся из справочника: у доллара и евро их два, у монеты шесть.
 * К ближайшему, а не вниз: хвост, отброшенный вниз, — это деньги
 * клиента, оставшиеся у сервиса сверх комиссии, которую он и так
 * назвал.
 */
describe('округление до знака', () => {
  it('округляет к ближайшему на заданном знаке', () => {
    // Контрольное число владельца: 795,63 минус 4,5% — 759,82665,
    // и на двух знаках это 759,83, а не 759.
    expect(roundTo(toAmount('759.82665'), 2)).toBe('759.83');
  });

  it('половину отправляет вверх, а не к чётному', () => {
    // Банковское округление «к чётному» дало бы 0,02 и 0,02 — правило,
    // которое человек с калькулятором не воспроизведёт.
    expect(roundTo(toAmount('0.025'), 2)).toBe('0.03');
    expect(roundTo(toAmount('0.035'), 2)).toBe('0.04');
  });

  it('не дописывает нулей к целому', () => {
    // Сумма уходит в заявку строкой, и «81000,00» рядом с «81000»
    // читались бы как два разных числа.
    expect(roundTo(toAmount('81000'), 2)).toBe('81000');
  });

  it('держит шесть знаков монеты', () => {
    // 50 000 / 85 — столько монет за рубли; седьмой знак единица, и
    // вниз.
    expect(roundTo(toAmount('588.23529411764705'), 6)).toBe('588.235294');
  });

  it('нулевым знаком округляет до целого', () => {
    expect(roundTo(toAmount('588.5'), 0)).toBe('589');
  });

  /*
   * Вверх на знаке валюты — там, где округляют сумму к оплате.
   *
   * POS-терминал ровняет то, что платит покупатель: рубли до целого,
   * монету до её знака. До целой монеты ровнять нельзя — она стоит под
   * сотню рублей, и «вверх» просило бы с покупателя лишнюю сотню.
   */
  it('вверх на знаке валюты не поднимает до целого', () => {
    expect(ceilTo(toAmount('55.6483031'), 6)).toBe('55.648304');
    expect(ceilTo(toAmount('55.648303'), 6)).toBe('55.648303');
    expect(ceilTo(toAmount('4999.01'), 0)).toBe('5000');
  });

  it('вверх не дописывает нулей к точному числу', () => {
    expect(ceilTo(toAmount('120'), 6)).toBe('120');
  });

  it('отвергает недопустимое число знаков', () => {
    expect(() => roundTo(toAmount('1.5'), -1)).toThrow(RangeError);
    expect(() => roundTo(toAmount('1.5'), 19)).toThrow(RangeError);
    expect(() => roundTo(toAmount('1.5'), 1.5)).toThrow(RangeError);
  });
});
