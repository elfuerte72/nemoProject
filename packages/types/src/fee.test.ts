 ...
 * фикс 5 $ до пятисот против 4,5 % дальше. Схема теперь сравнивает на
 * каждой границе комиссию до и после за одну и ту же сумму и отвергает
 * сетку, у которой вторая меньше первой. Слова отказа — те же, что
 * покажет форма панели до нажатия.
 */
describe('правила сетки', () => {
  ...
  it('отвергает сетку, в которой большая сумма стоит дороже меньшей', () => {
    expect(feeScheduleSchema.safeParse(BANK).success).toBe(false);
  ...
  it('отвергает фикс в валюте выдачи, растущий на границе', () => {
    // Фикс в валюте выдачи в доллары не перевести, но между собой две
    // ступени сравнить можно: двадцать евро после десяти — тот же провал.
    expect(
      feeScheduleComplaint([
        { upToUsd: Money.toAmount('2000'), rateBps: 330, fixedPayout: Money.toAmount('10') },
  ...
  it('с долей и фиксом на нижней ступени доля комиссии дальше только падает', () => {
    expect(feeScheduleComplaint(BANK_MONOTONE)).toBeNull();
