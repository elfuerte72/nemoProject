 ...
 *
 * Лежат пакетом, а не в одном из приложений, потому что рисуют их
 * двое: Mini App в выборе валюты и списке реквизитов, кабинет мерчанта
 * на табло курсов. Валюта — то же, что знак сервиса (`@nemo/brand`) и
 * чтение QR (`@nemo/qr`): двух правд о том, как она выглядит, быть не
 * должно. В `@nemo/ui` этому места нет — тот пакет делят панель и
 * кабинет, а Mini App собран своими поверхностями и его не берёт.
 *
 * Названия, страны и порядок живут в `@nemo/types` (`currencies.ts`): их
 * спрашивает и бот, у которого разметки нет вовсе. Добавить валюту —
 * значит дописать строку и там, и здесь; забыть про это можно
 * безнаказанно, но тогда клиент увидит серый кружок вместо флага.
 *
 ...
 * здесь, а в `@nemo/types` (`currencies.ts`): их знает и бот, а тянуть за ними
