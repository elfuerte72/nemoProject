...
export function createRapiraRateSource(options: RapiraOptions = {}): RateSource {
  ...
  return {
    async quote(pair: RatePair, at?: Date): Promise<RateQuote | null> {
      ...
      // Пересчёта через опорную валюту здесь нет — и не появится. Валюты,
      // которых биржа не котирует, считает фиатный источник: там опорная
      // валюта одна на весь справочник, а собирать цену из двух чужих
      // котировок «на всякий случай» значит выдавать за курс то, чего на
      // рынке никто не назвал.
      return null;
    },
  };
}
