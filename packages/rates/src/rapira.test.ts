...
/**
 * Пересчёта через опорную валюту у биржи нет: пару, которую она
 * котирует, она котирует прямо, а всё остальное считает фиатный
 * источник — там опорная валюта одна на весь справочник. Проверяется
 * поэтому обратное: цена, собранная здесь из двух котировок, за
 * настоящую не выдаётся.
 */
