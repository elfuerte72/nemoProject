...
/**
 * Котировки HTX P2P — источник курса юаня и евро.
 *
 * Названа владельцем прямо: «курс USD → CNY берём с HTX», а 27 августа
 * 2026 — и про евро: «с рубля получаем USDT по Rapira, дальше с HTX
 * вытягиваем конвертацию $ → EUR, это наш себес, и поверх него
 ...
 * торговать в юанях с 2021 года, — а евро она котирует, но не по той
 * цене, по которой сервис его получает.
 *
 * Доллар со стакана не берётся с 29 августа 2026: ТЗ владельца на
 * доллар говорит, что USDT и есть доллар и отдельной конвертации нет
 * (`pegged.ts`). Два дня до того он шёл отсюда, и цена его зависела от
 * того, сколько приманок по 1,20 $ за монету прошло в этот день фильтр.
 *
 ...
 *
 * Юань считается буквально по ТЗ владельца от 12 августа 2026: фильтр
 * по объёму заявки от 1 000 CNY, среднее арифметическое первых пяти
 * объявлений — тех самых, что владелец видит на экране HTX, поставив
 * тот же фильтр. Так его число сходится с нашим при сверке. Стакан
 * юаня чист — первые страницы держат мерчанты с тысячами сделок, — и
 * с 27 по 29 августа 2026 он считался медианой с фильтром истории; на
 * сверке это давало 6,64 против 6,652 у владельца, и правило вернули к
 * букве ТЗ.
 *
 * Евро — не по ТЗ, о нём владелец правила не давал, и стакан его не
 * чист. Замерено 27 августа 2026: верх занимали 1,02 € за USDT при
 * 0,86 на бирже — объявления мерчантов без единой сделки за месяц и с
 * нулевым процентом исполнения. Это приманки: сделка по ним не
 * состоится, а цена в среднем по пятёрке осталась бы. Поэтому у евро в
 * счёт идут объявления мерчантов с историей, а из пятёрки берётся
 * медиана, которую одна приманка утащить не может. Правило — у стакана,
 * а не у источника: две валюты, два ТЗ, два правила.
 */
...
interface Board {
  ...
  /**
   * Смотреть ли на историю мерчанта: объявления без исполнения и без
   * сделок за месяц — приманки, и на замусоренном стакане без этого
   * фильтра цены нет. В ТЗ по юаню фильтра нет, и юань считается по
   * букве: первые пять, какие есть.
   */
  readonly requireHistory: boolean;
  /**
   * Чем сводится пятёрка в одно число. Среднее — из ТЗ по юаню («СА из
   * первых 5»); медиана — защита евро от одной приманки, прошедшей
   * фильтр истории.
   */
  readonly aggregate: 'mean' | 'median';
}

/**
 * Стаканы, которые сервис читает: валюта выдачи → как её спрашивать и
 * считать.
 *
 * Номера сняты 14 августа 2026 (юань) и 27 августа 2026 (евро). Порог у
 * евро тот же, что у юаня, — тысяча в валюте стакана: владелец назвал
 * его один раз, для юаня, и второго правила не давал. Валюта, которой
 * здесь нет, у стакана не спрашивается вовсе — её знают соседи по
 * цепочке; доллар — единица по определению (`pegged.ts`).
 */
const BOARDS: Readonly<Record<string, Board>> = {
  CNY: { currency: '172', minOrder: '1000', requireHistory: false, aggregate: 'mean' },
  EUR: { currency: '14', minOrder: '1000', requireHistory: true, aggregate: 'median' },
};
 ...
 * десять проходят его же фильтр. Евро нужно больше: верх его стакана
 * занят приманками без истории, и пятёрка настоящих собирается со
 * второй-третьей страницы — замерено 27 августа 2026. Пять страниц —
 ...
 */
function qualifies(offer: HtxOffer, board: Board, threshold: Amount): boolean {
  ...
  if (!board.requireHistory) return true;
...
function topPrices(
  ...
  for (const offer of offers) {
    ...
    if (price === null || !qualifies(offer, board, threshold)) continue;
    prices.push(price);
  }
...
/**
 * Среднее арифметическое пятёрки — «СА из первых 5» по ТЗ владельца на
 * юань. Хранится частным: пять цен по сотой дают тысячные, и
 * округлять здесь значило бы округлить дважды — второй раз округляет
 * ядро, по правилу курса.
 */
function mean(prices: readonly Amount[]): Amount {
  const total = prices.reduce((sum, price) => Money.add(sum, price), Money.ZERO);
  return Money.divide(total, Money.toAmount(String(prices.length)));
}

/**
 * Медиана набранной пятёрки — для стакана, где среднее не защищено.
 *
 * У среднего одна приманка, прошедшая фильтр, весит пятую часть цены.
 * Замерено 27 августа 2026 на долларовом стакане: пятёрка с историей
...
export function createHtxRateSource(options: HtxOptions = {}): RateSource {
  ...
  async function fetchRate(board: Board): Promise<Amount> {
    ...
    for (let page = 1; page <= MAX_PAGES && prices.length < TOP_OFFERS; page += 1) {
      ...
      prices = topPrices(offers, board, threshold, prices);
    }
    ...
    return board.aggregate === 'mean' ? mean(prices) : median(prices);
  }

  /*
   * У каждого стакана свой кэш: юань и евро — два разных запроса, и
   * молчание одного не должно ронять другой. Имя провайдера в журнале
   * — со стаканом, иначе по строке «HTX не ответила» не понять, какая
   * валюта осталась без курса.
   */
  ...
  return {
    async quote(pair: RatePair, at?: Date): Promise<RateQuote | null> {
      // Спрошенные стаканы — про две пары и одну их сторону. Всё
      // остальное здесь молчит, не сходив в сеть: чужие пары спрашивают
      // у соседей по цепочке, доллар — единица по определению.
