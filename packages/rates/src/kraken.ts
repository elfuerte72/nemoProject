...
/**
 * Через сколько котировка просит обновления. Минута, как у соседних
 * бирж, и по той же причине: этим же сроком снимок обновляет себя сам,
 * без клиента, и десять секунд означали бы круглосуточный стук в чужой
 * публичный API. Доллар с евро против USDT ходят долями процента —
 * минута тут не видна.
 */
...
/**
 * Насколько старую котировку ещё можно показывать. Час, как у соседних
 * бирж и по той же причине: срок этот про молчание биржи, а не про
 * паузу между клиентами. По такому курсу подают заявку, а она
 * обязательство.
 */
...
export function createKrakenRateSource(options: KrakenOptions = {}): RateSource {
  ...
  async function fetchTickers(): Promise<Map<string, Book>> {
     ...
     * оставляет прежний снимок жить положенный ему час.
     */
    if (book.size === 0) {
