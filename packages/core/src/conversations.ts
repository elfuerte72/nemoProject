...
export async function listConversations(
   ...
   *
   * Но только пока просьбе не ответил человек. До 4 сентября 2026 тема
   * висела на разговоре навсегда, и «Отель» стоял у «привет» месяц
   * спустя — читалось как тема этого сообщения. Ответ сотрудника после
   * просьбы снимает её: разобранная просьба — история, а не работа.
   * Ответ помощника — нет: оплатить отель он не может и зовёт человека,
   * а просьба должна дождаться этого человека с пилюлей.
   */
  ...
  const answered = ctx.db.$with('answered').as(
    ctx.db
      ...
      .from(clientMessages)
      .where(isNotNull(clientMessages.authorStaffId))
      .orderBy(clientMessages.clientId, desc(clientMessages.seq)),
  );
