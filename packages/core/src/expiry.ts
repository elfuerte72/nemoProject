...
import { merchantAddressBook } from './merchants.js';
 ...
 *
 * Вынесена наружу, потому что по ней же воронка мерчанта отделяет
 * просроченные от отменённых: своего состояния у просрочки нет, и
 * различает их только причина. Набранная второй раз строкой, она
 * разошлась бы с этой при первой правке слов — и воронка молча
 * показала бы ноль просроченных.
 */
export const EXPIRED_REASON = 'Истёк срок оплаты по названному курсу';
...
export async function expireUnpaidExchangeRequests(
  ...
  return ctx.db.transaction(async (tx) => {
    ...
    const book = await merchantAddressBook(tx, expired);
    return expired.map((row) => cancelledNotification(row, book.for(row)));
  });
}
...
export async function warnAboutExpiringExchangeRequests(
  ...
  return ctx.db.transaction(async (tx) => {
    ...
    const book = await merchantAddressBook(tx, warned);
