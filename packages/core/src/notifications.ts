...
/**
 * Подпись к файлу, который менеджер отправляет клиенту.
 *
 * Файл уходит не уведомлением, а прямой отправкой — байты нельзя
 * описать текстом, — но подписывается тем же, чем ответ словами: в
 * одном чате говорят бот, помощник и человек, и присланный без подписи
 * файл читается как ещё одно движение автомата. Слов при этом может не
 * быть вовсе: квитанция бывает сама себе ответом, и тогда от подписи
 * остаётся одно имя — без двоеточия, за которым ничего не следует.
 */
export function operatorCaption(body: string | undefined): string {
  const words = body?.trim();
  return words ? `${OPERATOR_PREFIX}${words}` : OPERATOR_PREFIX.replace(/:\s*$/, '');
}
