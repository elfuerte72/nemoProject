...
/**
 * Строка записи — в то, что видно без расшифровки. Наружу уходит
 * представление без шифрованных полей: конверт бесполезен без
 * приватного ключа, но и отдавать его клиентскому приложению незачем.
 *
 * Открыта модулю заявок: получателя заявки он отдаёт тем же видом, что
 * и список, и второй сборщик разошёлся бы с первым на первом новом поле.
 */
