...
describe('кто ведёт заявку', () => {
  ...
  /*
   * И в самой карточке тоже: открыв чужую заявку, менеджер видит, к кому
   * идти. Без соединения имя приходило бы пустым, а экран говорил бы
   * «другой менеджер» — то есть ничего.
   */
  it('называет менеджера и в карточке заявки', async () => {
    const id = await givenNewRequest();
    await core.claimExchangeRequest(petr, id);

    const seen = await core.getExchangeRequestForStaff(anna, id);

    expect(seen.assignedManagerName).toBe('Пётр');
  });
