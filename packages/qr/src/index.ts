...
/**
 * Чтение QR из картинки — на устройстве того, кто заводит запись.
 *
 * Реквизит приносят картинкой: PromptPay-QR из тайского банка или
 * кошелька, QR приёма Alipay. Картинка расшифровывается здесь, в
 * браузере, и на сервер уходит только строка — шифрованной, как номер
 * карты; сама картинка устройство не покидает (docs/adr/0012).
 *
 * Свой пакет, а не модуль приложения: читают QR и Mini App, и кабинет
 * мерчанта, и правило одно — вплоть до размеров, до которых ужимается
 * картинка. Библиотека — чистый JS без зависимостей, и грузится модуль
 * лениво, в момент показа поля: первый экран его не везёт.
 *
