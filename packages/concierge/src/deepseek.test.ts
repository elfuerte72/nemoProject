...
describe('ответ модели', () => {
  ...
  it('просит человека и сигнальной строкой в конце', async () => {
    /*
     * Первой строкой её велено ставить, но модель дешёвая и ставит где
     * придётся. Считанная только по первой, такая просьба проходила
     * мимо — и слово «МЕНЕДЖЕР» уезжало клиенту хвостом ответа.
     * Проверено на живом провайдере 7 августа 2026, ответ из ленты
     * dev-бота.
     */
    const { fetch } = givenModel(
      'Здравствуйте. По вашей заявке выданы реквизиты, она ждёт оплаты.\n\nМЕНЕДЖЕР',
    );
