...
describe('запрос', () => {
  ...
  it('не просит у провайдера того, чего у него нет', async () => {
    // Адаптивное мышление, `effort` и структурный вывод — возможности
    // Anthropic, а не совместимого слоя: присланные ему, они вернулись
    // бы отказом на каждое сообщение клиента.
    const { fetch, sent } = givenModel('Ответ');
