 ...
 * строка на каждого предка до пятой линии (docs/adr/0021). Сколько из
 ...
 * позвал в программу поимённо (блогер, чат, канал). Своей сущности
 * владельца ему не заведено намеренно: приводит он своей реферальной
 * ссылкой, и вторая цепочка рефералов рядом с первой означала бы два
 * счёта одних и тех же баллов.
 *
 * Ставка здесь не живёт: личная лежит в `client_referral_rates` и
 * работает по старшинству «личная → уровень → базовая». Две правды о
 * проценте одного человека разошлись бы при первой правке.
 */
export const ambassadors = pgTable('ambassadors', {
  clientId: bigint('client_id', { mode: 'bigint' })
    .primaryKey()
    .references(() => clients.telegramUserId, { onDelete: 'cascade' }),
  /** Как администратор подписал: «Канал „Пхукет за рубль“». */
  title: text('title').notNull(),
  /** Зачем завели — свободным текстом, для того, кто придёт разбираться. */
  note: text('note'),
  ...
  createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
  /**
   * Когда первый раз вошёл. Пусто — ключ в программу выдан, а человек
   * до кабинета не дошёл: это разные состояния, и администратор,
   * которого спросят «почему он ничего не видит», должен их различать.
   */
   ...
   * когда снятый придёт спросить, почему его не пускают.
   */
  revokedAt: timestamp('revoked_at', { withTimezone: true }),
});
