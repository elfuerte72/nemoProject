 ...
 * Остальные семь валют он только выдаёт: чтобы принимать платёж в
 * батах, сервису нужны свои реквизиты в Таиланде, а их нет. Юань и бат
 * при этом выдаются и на руки — там, где у сервиса есть касса;
 * остальные пять только переводом. Отдать за них можно и USDT, и рубли — обе стороны, которые
 * сервис принимает. Валюты сверх этих девяти скрипт удаляет: экран не должен
...
/**
 * Из них те, что сервис выдаёт ещё и на руки: юань и бат.
 *
 * Список отдельный, потому что наличная выдача — это не свойство
 * валюты, а наличие кассы в стране. Она есть в Китае и Таиланде и
 * появится где-то ещё; остальные пять валют выдаются только переводом.
 *
 * Курса у наличной заявки, как и раньше, нет, пока администратор не
 * заведёт наличную сетку ставок на эту валюту: наличный обмен стоит
 * сервису другого — касса, встреча, риск, — и подставлять вместо его
 * цены безналичную нельзя. До первой заведённой ступени клиент видит
 * «курс назовёт менеджер», а заявку подаёт как обычно.
 */
const CASH_PAYOUT = ['CNY', 'THB'];
 ...
 *
 * У валют выдачи направления только в одну сторону: принимать платежи в
 * них сервис не умеет. Юань и бат он выдаёт и наличными, остальные пять
 * — переводом.
 */
const PAIRS = [
  ...
  /*
   * Валюты выдачи сервис отдаёт и за USDT, и за рубли: клиент приходит
   * за батами с рублями на карте, а не с монетой на кошельке, и
   * заставлять его менять дважды значит терять его на первом шаге.
   * Прямой цены у такой пары нет ни у одного провайдера — она
   * собирается через USDT (`cross.ts` в `@nemo/rates`), тем же путём,
   * которым идут деньги сервиса.
   */
  ...PAYOUT_ONLY.flatMap((code) => [
    { fromCode: 'USDT', toCode: code, kind: 'electronic' as const },
    { fromCode: 'RUB', toCode: code, kind: 'electronic' as const },
  ]),
  /*
   * Наличная выдача там, где у сервиса есть касса. Направление всё так
   * же одностороннее: наличные юани и баты сервис отдаёт, а принимать в
   * них платежи по-прежнему не умеет — для этого нужны свои реквизиты в
   * стране.
   */
  ...CASH_PAYOUT.flatMap((code) => [
    { fromCode: 'USDT', toCode: code, kind: 'cash' as const },
    { fromCode: 'RUB', toCode: code, kind: 'cash' as const },
  ]),
];
