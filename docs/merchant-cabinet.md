...
**Счета.** Устройство — по разделу «Счета» образца Love&Pay, слова —
владельца со звонка 8 сентября. Плитки «Всего счетов» с суммой и ходом
по дням, «Ожидают» («все обработаны» при нуле), «Оплаченные счета» с
возвратами и ходом по дням (в них и возвращённые: деньги по ним
приходили), «Оборот в RUB» — оплаченное минус возвраты, с конверсией,
«По валютам» без сложения, столбиком со значками валют; валюта сумм
выбирается пилюлей «Оборот в [RUB ▾]» над плитками — список всех валют
сервиса, та же пилюля, что в POS-терминале,
возврат переводится в валюту оплаты долей самого счёта и ровняется так
же, как сумма к оплате. Состояния Ожидает, Оплачен, Истёк, Отменён,
Возвращён: возвращённым счёт становится, когда деньги вернули целиком
и возврат исполнен, — считается это при чтении, как истечение, — а
частичный возврат оставляет его оплаченным с отметкой в строке. Отбор
живёт в адресе и разбирается одним правилом со страницей и выгрузкой:
поиск по номеру, период («За всё время», «Сегодня», 7 и 30 дней, свой),
переключатель «Счета всех сотрудников» (выключенный оставляет счета
того, кто смотрит). Сделка в строке — двумя колонками, «Покупатель
платит» и «Покупатель получает», со значками валют. Отмеченные строки
можно выгрузить, отметить оплаченными (только ждущие оплаты) или
удалить (только те, по которым денег не было) — с подтверждением.
«По валютам» раскрывается списком всех валют сервиса. Под QR — в
терминале и в карточке — «Скопировать ссылку» на оплату: та же, что в
QR, и живёт столько же. «Вид таблицы» над таблицей справа — личные
колонки, плотная таблица и число строк на странице (10, 25, 50, 100);
список режется на страницы «1–25 из 55», у строки кнопка «Подробнее».
«CSV» отдаёт весь отбор, отмеченные галочкой строки выгружаются
отдельно. Главная кнопка страницы «Создать счёт» — справа от заголовка,
с плюсом; ведёт в POS-терминал: второй формы с той же ценой нет.
Карточка счёта: номер с состоянием и датой создания, сумма и
эквивалент по записанному курсу, QR пока счёт ждёт денег,
идентификаторы счёта и заказа у провайдера с копированием, таблица
«Платёж у провайдера», верификация, лента «Что происходило», действия.
...
- Перед показом проверить, что кабинет отвечает и что на нём та версия,
  о которой рассказываете. На вечер 22 сентября 2026 оба контура на
  одном коде: dev и прод на версии d679b35, примеры включены только на
  dev.
