...
**Счета.** Устройство — по разделу «Счета» образца Love&Pay. Плитки
«Всего счетов» с суммой и ходом по дням, «Ожидают» («все обработаны»
при нуле), «Оплачено» с возвратами и ходом по дням, «Чистый оборот» —
оплаченное минус возвраты, с конверсией, «По валютам» без сложения;
суммы — в валюте, выбранной чипом («Суммы в RUB»), возврат переводится
в валюту оплаты долей самого счёта. Состояния Ожидает, Оплачен, Истёк,
Отменён, Возвращён: возвращённым счёт становится, когда деньги
вернули целиком и возврат исполнен, а частичный возврат оставляет его
оплаченным с отметкой в строке. Отбор живёт в адресе: поиск по
номеру, период («За всё время», «Сегодня», 7 и 30 дней, свой),
переключатель «Счета всей команды» (выключенный оставляет счета того,
кто смотрит). «Поля» — личные колонки, плотная таблица и число строк на
странице (10, 25, 50, 100); список режется на страницы «1–25 из 55».
«Выгрузить» отдаёт CSV всего отбора, отмеченные галочкой строки
выгружаются отдельно. Кнопка «Создать счёт» ведёт в POS-терминал:
второй формы с той же ценой нет.
