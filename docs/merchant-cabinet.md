...
становится той, от которой считают встречную. Валюту выбирают там же,
в строке её суммы: нажатие на значок раскрывает список с флагом, кодом
и страной. Галочка «Требовать верификацию» под калькулятором. Сумма к
...
**Наценка владельца** — в строке «Покупатель платит», у числа, которое
она поднимает: от 0 до 100 % поверх курса сервиса, его доход с продажи
у стойки, записывается в каждый счёт вместе с курсом. Нажал, вписал,
сохранил — цена на экране пересчиталась. Оператор наценку видит, но не
правит, и маршрут откажет ему тем же правом: знать, из чего сложилась
цена, продавцу нужно, менять её — нет.
...
| 22 сентября | Расчёт в терминале собран калькулятором Mini App: две строки, набор в любой из них, курс на черте. Валюта выбирается в строке своей суммы, наценка стоит в строке «Покупатель платит». |
