...
«По валютам» без сложения; валюта сумм выбирается чипом «Оборот в …»,
...
того, кто смотрит). «Поля» — личные колонки, плотная таблица и число
строк на странице (10, 25, 50, 100); список режется на страницы «1–25
из 55», у строки кнопка «Подробнее». «CSV» отдаёт весь отбор,
отмеченные галочкой строки выгружаются отдельно. Кнопка «Создать счёт»
ведёт в POS-терминал: второй формы с той же ценой нет.
