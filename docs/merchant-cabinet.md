...
Меню из пяти групп отвечает на вопросы по мере их появления: что у
меня происходит, чем это делать программно, кто и откуда вошёл, кто я
такой и куда писать. Группы «Разработчикам» и «Безопасность» устроены по
образцу Love&Pay (24 сентября 2026); антифрод по чекам и KYC образца не
взяты — у сервиса нет плательщика, которого проверять.
...
| Разработчикам | Документация | Договор API, тот же файл, что проверяют тесты; коды ошибок и заголовки ответа таблицами | владелец |
| Разработчикам | API | Ключи; проверки запроса (подпись, разрешённые адреса); ограничения; разрешённые адреса с подсказкой «откуда звали за сутки»; пример на Node.js, Python, PHP и curl | владелец |
| Разработчикам | Вебхуки | Точки, события, история доставок, пробная доставка, пауза; «Как встроить» — кнопкой в шапке | владелец |
| Разработчикам | Журнал вызовов | Каждый вызов API с узнанным ключом, включая отвергнутые; поиск по пути и x-request-id, метод, адрес клиента, карточка вызова | владелец |
| Разработчикам | Изменения API | Что менялось в договоре и что сделать интеграции | владелец |
| Разработчикам | Песочница | Ссылка на тестовый контур с ключами sk_test_ | владелец |
| Безопасность | Сессии | Свои входы: устройство, адрес, была активность; отключить одну или все, кроме этой | всем |
...
4. Плитки: Оборот (отдано, только RUB и USDT), Выдано (получили
   получатели, в каждой валюте выдачи) и Средний чек (с медианой) по
   валютам со значками, Подано, Исполнено, Отменено, До исполнения, Конверсия,
...
10. Тройкой: Валюты (отдано и выдано), До исполнения (среднее,
...
| Пароль | Хранится хешем argon2id. |
| Сессии | Каждый вход — запись в базе, кука несёт её номер (ADR-0025). Человек видит свои входы в «Сессиях» и отключает любой по одному; выход гасит свою запись, и копия куки после него не пускает. Смена пароля или закрытие доступа обрывает все сессии разом — поколением. |
| Ключ API | В базе лежит только хеш SHA-256; секрет показывается один раз и отзывается по одному. По желанию владелец включает подпись HMAC каждого запроса и список разрешённых адресов: с ним ключ бесполезен вне серверов мерчанта. Адрес за Traefik подделать нельзя — он вычищает `X-Forwarded-For` недоверенных клиентов. |
| Пределы | На ключ API — сто запросов в минуту и тысяча в час; остаток — в заголовках ответа. На вход и на незнакомые ключи — счётчик попыток по адресу. |
...
| Проверка личности покупателя (KYC), KYC API, антифрод по чекам | Есть только галочка на счёте в терминале, проверку делал бы провайдер приёма. Своего провайдера проверки нет, а чек сверять не с чем: деньги покупателя сервис не принимает. Разделы образца «Антифрод», «KYC» и «KYC API» поэтому не заведены. |
| Подтверждённые домены и API v3 с сертификатами — из раздела «API» образца | Вызовы идут с серверов мерчанта, домен в них ничего не удостоверяет — удостоверяет адрес, и список адресов есть. Второй контур с сертификатами и шифрованием тела поверх TLS и HMAC ничего не добавил бы при нашем объёме. |
| Тела запроса и ответа в журнале вызовов | В телах реквизиты получателей, а открытым текстом клиентский контур их не держит. Вызов находится по x-request-id, тело видел сервер мерчанта. |
