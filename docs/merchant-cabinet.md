...
табло «Курсов», — и калькулятор под ними: две строки, «Покупатель
платит» и «Покупатель получает», крупные суммы одного кегля, значок
валюты у каждой, курс с наценкой на черте между ними. Устройство то
же, каким клиент меняет деньги в Mini App; набирают в любой строке, и
набранная становится той, от которой считают встречную. Галочка
«Требовать верификацию» под калькулятором. Сумма к оплате округляется
вверх до целого рубля. Блок «Как это устроено» показывает путь денег
на набранной сумме — платит, по курсу, получает, — тем же приёмом, что
на табло курсов. После «Создать счёт» на месте
...
**Наценка владельца** — кнопкой над терминалом: от 0 до 100 % поверх
курса сервиса, его доход с продажи у стойки, записывается в каждый
счёт вместе с курсом. Нажал, вписал, сохранил — цена на экране
пересчиталась. Кнопку видит только владелец; оператору за стойкой её
не показывают, и маршрут откажет ему тем же правом.
...
| 22 сентября | Расчёт в терминале собран калькулятором Mini App: две строки, набор в любой из них, курс на черте. Наценка задаётся кнопкой на самом экране терминала. |
...
5. POS-терминал: набрать сумму в любой строке калькулятора и показать,
   как считается встречная, раскрыть «Как это устроено» с путём денег,
   создать счёт, показать QR и отсчёт, нажать «Покупатель заплатил» и
   увидеть «Оплачено!»; сменить наценку кнопкой над терминалом — цена
   пересчитается сразу.
